...
private final class DefaultBloomFilter[A] private (
    ...
    val hashFunctions: IndexedSeq[A => Int],
    var _size: Int,
    val buffer: collection.mutable.BitSet
  ...
  override def add(value: A): this.type = {
    val indices = indicesOf(value)
    if (!contains(indices)) {
      _size += 1
      indices.foreach(buffer.addOne)
    }
    this
  }
  ...
  override def clone(): BloomFilter[A] = {
    new DefaultBloomFilter[A](settings, hashFunctions, _size, buffer.clone())
  }
...
private object DefaultBloomFilter {
  ...
  def apply[A](settings: BloomFilterSettings)(implicit
    ...
    new DefaultBloomFilter[A](
      ...
      hashFunctions,
      0,
      new collection.mutable.BitSet(settings.numOfBits)
    )
  }
